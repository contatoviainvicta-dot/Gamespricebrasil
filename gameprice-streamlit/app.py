...
/* Container centralizado estilo ITAD */
.main .block-container { max-width:960px; padding:1.5rem 2rem; margin:0 auto; }

/* Deal row horizontal estilo ITAD */
.deal-row {
    display:grid;
...
.store-pill {
    ...
    background:#f0f2f4; color:#444; border-left:3px solid #aaa;
}
...
.badge-lowest { background:#c6430a; color:#fff; }
.badge-soon  { background:#e6a817; color:#fff; }
.section-hd {
    font-size:1rem; font-weight:700; color:#1b2838;
    border-left:4px solid #4c9d4c; padding-left:10px;
    margin:18px 0 10px;
}
/* Ranking table */
.rtable { width:100%; border-collapse:collapse; font-size:.85rem; }
.rtable th { background:#f4f6f8; padding:6px 10px; text-align:left; font-size:.75rem; color:#666; border-bottom:1px solid #ddd; }
...
def jogos_com_desconto(loja="Todas",plat="Todas",min_desc=0,limite=40):
    q = (sb.table("v_game_offers")
         ...
         .order("discount_percent",desc=True).limit(limite))
...
def deal_card(j:dict, idx:int):
    ...
    elif pct >= 80: badge = f'<span class="badge badge-pct">-{pct}%</span>'
...
<div class="deal-row">
  ...
  <div class="deal-price">
    <span class="price-now">{preco_str}</span>
    {old_str}
  </div>
...
if pagina == "🏠 Deals":
    ...
    if filtro_preco != "Qualquer":
        preco_max = float(filtro_preco.replace("R$ ","").replace(".","").replace(",","."))

    st.markdown('<div class="section-hd">🔥 Melhores deals agora</div>',
                unsafe_allow_html=True)

    deals = jogos_com_desconto(loja=filtro_loja,plat=filtro_plat,
                                min_desc=filtro_desc,limite=60)
    if preco_max:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if nome_f:
        jogos_cat = [j for j in jogos_cat if nome_f.lower() in j.get("title","").lower()]
    if ord_f=="Menor preço":
        jogos_cat = sorted(jogos_cat,key=lambda x:float(x.get("price") or 9999))
    elif ord_f=="Maior desconto":
        jogos_cat = sorted(jogos_cat,key=lambda x:x.get("discount_percent") or 0,reverse=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if epic.get("current"):
        st.caption("Grátis: "+" · ".join(g["title"] for g in epic["current"]))
