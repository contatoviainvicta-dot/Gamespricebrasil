...
Mercado Livre / Amazon: scaffold (precisam de token/credencial).
"""
import httpx
...
def fetch_steam(appid: str, cc: str = "br", lang: str = "portuguese") -> dict | None:
    ...
    # Steam as vezes retorna data como lista — pega o primeiro elemento
    data = raw.get("data", {})
    ...
    if po is None:
        # Jogo gratuito (free-to-play) ou sem preco listado
        return {
            "price": 0.0,
            "old_price": None,
            "discount_percent": 0,
...
def fetch_mercadolivre(external_id: str) -> dict | None:
    """Scaffold: API do Mercado Livre exige token OAuth. Implementar depois."""
    return None
